...
import de.osca.android.coworking.presentation.args.CoworkingDesignArgs
import javax.inject.Singleton

@Module
@InstallIn(SingletonComponent::class)
class CoworkingDesignModule {

    @Singleton
    @Provides
    fun providesCoworkingDesignArgs(): CoworkingDesignArgs {
        return object : CoworkingDesignArgs {
            /**
             * these are the values that must be set
             * this is used by the module and all screens with the same viewModel
             */
            override val inCard: Boolean = true
            override val showInfoElement: Boolean = true
            /**
             * -> overrides that must be set
             */
            override val vShowMoreOption: Boolean = true
            override val vModuleTitle: Int = R.string.coworking_title
            /**
